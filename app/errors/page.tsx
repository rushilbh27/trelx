  ...
  return (
    <main className="mx-auto max-w-7xl px-5 py-8">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-[0.24em] text-emerald-300">Failure intelligence</p>
        ...
        <Link href="/calls" className="border border-white/10 bg-black px-4 py-3 text-xs font-black uppercase tracking-[0.14em] text-zinc-300 hover:border-emerald-300 hover:text-white">
          calls
        </Link>
      </div>
      ...
      <section className="mb-6 grid gap-3 md:grid-cols-4">
        {[...counts.entries()].sort((a, b) => b[1] - a[1]).slice(0, 4).map(([type, count]) => (
          <div key={type} className="border border-white/10 bg-black p-4">
      ...
      <section className="grid gap-3">
        {rows.map((row) => (
          ...
          >
            ...
            <blockquote className="mt-3 border-l-2 border-emerald-300 pl-4 text-sm text-zinc-200">
              {row.quote ?? "No quote captured."}
            </blockquote>
          </Link>
        ))}
        {rows.length === 0 ? <div className="border border-white/10 bg-black p-8 text-sm text-zinc-400">No errors detected yet.</div> : null}
      </section>
    </main>
  );
}
